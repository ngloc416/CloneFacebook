  ...
  return (
    <View style={styles.container}>
      <Avatar
        ...
        source={{ uri: comment.poster.avatar }}
      />
      <View style={styles.centerContainer}>
        <View style={styles.contentContainer}>
          <TouchableOpacity>
            <Text style={styles.name}>{comment.poster.name}</Text>
          </TouchableOpacity>
        ...
        <View style={styles.toolContainer}>
          {time < 1 * 60 ? <Text style={styles.createAt}>Vừa xong</Text> : null}
          {time >= 1 * 60 && time < 60 * 60 ? (
            <Text style={styles.createAt}>{Math.floor(time / 60)} phút</Text>
          ) : null}
          {time >= 1 * 60 * 60 && time < 24 * 60 * 60 ? (
            <Text style={styles.createAt}>{Math.floor(time / 3600)} giờ</Text>
          ) : null}
          {time >= 24 * 60 * 60 && time < 30 * 24 * 60 * 60 ? (
            <Text style={styles.createAt}>{Math.floor(time / 86400)} ngày</Text>
          ) : null}
